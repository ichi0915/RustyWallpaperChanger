...
fn download_image(url: &Url) -> Result<String> {
    ...
    // reqwest::get(url.as_str())?.copy_to(&mut file)?;		//Original
	let _res = reqwest::get(url.as_str());					//Esta compila pero no jala descargar las imagenes

	// let body = reqwest::get("https://www.rust-lang.org").await?.text().await?;
	// reqwest::get(url.as_str()).unwrap().read_to_string(&mut file);

	println!("Imagen descargada en: {:?}", file_path.to_str());
